...
class Game(arcade.Window):
    ...
    def __init__(self):
        ...
        self.start_thread=True  #flag boolean tarifmikonim baray faal va gheyr faal kardn thread ha 
        self.end_thread=False

        self.my_thread=threading.Thread(target=self.add_enemy) # self mikhad chon ozv hamin clase
    ...
    def add_enemy(self):
        while True:
            time.sleep(5)
            speed=3  #speed dar halat thread bedon taghir gozashtm 
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.UP:
            ...
            self.me.change_x = -1
        elif key == arcade.key.M:  #J crotate to right
            self.me.change_angle = 1
        elif key == arcade.key.N:  #F rotate to left 
            self.me.change_angle = -1
